...
/**
 *<code>PhysicsManager</code> is responsible for updating all properties of the
 * DinamicEntities of the Game
 * 
...
public class PhysicsManager {
	...
	/**
	 * Contains the <code>IDinamicEntity's</code> of the Game to be updated in the next
	 * iteration.
	 */
	private ArrayList<IDynamicEntity> entities;

	/**
	 * Retrieve the <code>IDynamicEntity's</code> ArrayList of the Game to be updated in
	 * the next iteration.
	 * 
	 * @return the <code>IDynamicEntity's</code> ArrayList of the Game to be updated in
	 *         the next iteration.
	 */
	...
	private void updateTranslation(IDynamicEntity entity) {
		...
		Vector3f actualPosition = entity.getPosition().clone();
		entity.setPosition(actualPosition.addLocal(tempVector));
	...
	private void updateVelocity(IDynamicEntity entity) {
		Vector3f velocity = entity.getForce().divideLocal(entity.getMass())
				.multLocal(this.rate);
	...
	/**
	 * Actualiza el <code>PhysicsManager</code>. itera aplicando las propiedades
	 * fisicas mientras le quede tiempo para hacerlo si completo la
	...
	/*
	 * public void update(float interpolation) { // Return if there is no
	 * entities need to be updated. if(this.entities.size() <= 0) return; //
	 * Perform as many iterations as needed. this.time += interpolation;
	 * 
	 * if(this.time >= this.rate) this.updated = true; else this.updated =
	 * false; while(this.time >= this.rate) { for(IDynamicEntity entity :
	 * this.entities) { this.applyForce(entity); this.updateVelocity(entity);
	 * this.updateTranslation(entity);
	 * ViewManager.getInstance().markForUpdate(entity);
	 * 
	 * } this.time -= this.rate; }
	 * System.out.println(((Player)this.entities.get(0)).getPosition());
	 * if(!this.updated) { this.entities.clear(); this.updated = false; } }
	 */
}