...
public final class ChannelConteiner {
	...
	public ChannelConteiner() {
		channelsByType = Collections
				.synchronizedMap(new Hashtable<String, Set<ClientChannel>>());
	}
	...
	public void addChannel(final ClientChannel channel) {
		...
		String channelType = ChannelNameParser.parseChannelType(channel
				.getName());
		/** Se obtienen todos los canales del mismo tipo. */
		Set<ClientChannel> channels = this.channelsByType.get(channelType);
		/**
		 * Si no habia ningun canal del mismo tipo, debo crear el conjunto y
		 * agregalo al hash.
		 */
		if (channels == null) {
			channels = Collections
					.synchronizedSet(new HashSet<ClientChannel>());
