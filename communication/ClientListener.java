 ...
 * y de todos los canales a los cuales este estara subscripto.
 * 
 * @author lito
 */
public class ClientListener implements	ClientChannelListener,
										SimpleClientListener {
	
	/** El {@link Logger} para esta clase. */
    private static final Logger LOGGER =
        Logger.getLogger(ClientCommunication.class.getName());
	
	/**
	 * Cada vez que un mensaje es recivido por el canal <I>channel</I>
	 * al cual se le asocio esta instancia como listener, se dispara
	 * este metodo.<BR/>
	 * Para procesar el mensaje internamente, se invoca el metodo
	 * {@link #processIncomeMsg(ByteBuffer)}.
	 * 
	 * @param channel El canal por donde provino el mensaje.
	 * 
	 * @param bbMsg El mensaje recibido como buffer de bytes.
	 * 
	 * @see ClientChannelListener#receivedMessage(
	 * ClientChannel arg0,ByteBuffer arg1)
	 */
    @Override
	public final void receivedMessage(
			final ClientChannel channel, final ByteBuffer bbMsg) {
		this.processIncomeMsg(bbMsg);
	}
	
	/**
	 * Se crea la instancia de {@link IMessage} corespondiente al mensaje
	 * a travez de {@link MessageFactory}, luego se crea la instancia
	 * a {@link IProcessor} correspondiente (mediante la
	 * {@link MsgProcessorFactory}) al tipo de mensaje para que
	 * procese el mensaje.<BR/>
	 * En caso de ocurrir una excepcion del tipo
	 ...
	 * {@link #LOGGER}.
	 * 
	 * @param bbMsg El mensaje a procesar.
	 */
	 ...
	 * {@linl ChannelContainer}, y remueve del mismo el canal
	 * <I>channel</I> del cual el sevidor removio a este cliente.
	 *  
	 * @param channel El canal del cual el servidor a removido
	 * al cliente.
	 * @see ClientChannelListener#leftChannel(ClientChannel arg0)
	 */
	@Override
	public final void leftChannel(final ClientChannel channel) {
		GameContext.getClientCommunication().
			getChannelConteiner().removeChannel(channel);
	}

	/** 
	 * El metodo es invocado por {@link SimpleClient} cuando el servidor
	 * agrega al cliente a la canal <I>channel</I>.<BR/>
	 * Internamente, se solicita a {@link GameContext} la instancia de
	 * {@link ClientCommunication}, a este le pide su
	 * {@linl ChannelContainer}, y agrega al mismo el canal
	 * <I>channel</I> al cual el sevidor agrego a este cliente.
	 * 
	 * @param channel El canal al que el servidor suscribio a este cliente.
	 * 
	...
	public final ClientChannelListener joinedChannel(
		...
		GameContext.getClientCommunication().
			getChannelConteiner().addChannel(channel);
		return this;
	}

	/** 
	 * El metodo se dispara al recibir un mensaje directo dede el
	 * servidor.<BR/>
	 * Para procesar el mensaje internamente, se invoca el metodo
	 * {@link #processIncomeMsg(ByteBuffer)}.
	 * 
	...
	/** 
	 * Este metodo es disparado por {@link SimpleClient} al
	 * momento que se esta intentando reconectar al serivor
	 * darkstar.
	 * Internamente:<BR/>
	 * <CODE>
	...
	/** 
	 * Este metodo es disparado por {@link SimpleClient} al
	 * momento que se reconecta al serivor darkstar.
	 * Internamente:<BR/>
	 * <CODE>
	 *  AbstractMessage msg = new EmptyMessage();<BR/>
	...
	/** 
	 * Este metodo es disparado por {@link SimpleClient} al
	 * momento que se deconecta del serivor darkstar.
	 * Internamente:<BR/>
	 * <CODE>
	 ...
	 * 	MsgProcessorFactory.getInstance().createProcessor
	 * 	(msg.getType()).process(msg);.</CODE>
	 * @param gracefull No se utiliza.
	 * 
	 * @param razon La justificacion de porque se desconecto.
	 * 
	...
	/**
	 * Este metodo es solicitado por {@link SimpleClient} al
	 * momento de intentar logease a un servidor.
	 *  
	...
	/** 
	 * Este metodo es disparado cuando el cliente se logeo en un
	 ...
	 *  (msg.getType()).process(msg);.</CODE>
	 * 
	 * @param razon Una explicacion de por que fallo el login.
	 * 
